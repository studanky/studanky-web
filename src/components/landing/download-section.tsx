 ...
 * atmosphere. Phones get the single /download button (the QR makes no sense on the
...
export function DownloadSection({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  return (
    <section
      id="download"
      className="relative flex flex-col justify-center overflow-hidden px-4 py-20 sm:px-6 sm:py-28 min-h-svh lg:px-8"
    >
      ...
      <Reveal className="mx-auto flex w-full max-w-3xl flex-col items-center gap-8 text-center">
        ...
        <div className="hidden flex-col items-center gap-4 md:flex">
          <span className="rounded-3xl border border-border/70 bg-white p-5 shadow-xl shadow-primary/10">
            <DownloadQrCode label={download.qrAlt} className="size-44 text-black" />
          </span>
